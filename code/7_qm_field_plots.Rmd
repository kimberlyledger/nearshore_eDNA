...
### aside to look closer at simple means ###
```{r}
t1 <- mock1$env %>% 
...
t1 %>% group_by(community) %>% summarize(total = sum(simple.Mean, na.rm = T)) 

mock1$env %>% 
  group_by(community) %>% 
  summarize(total = sum(propReads, na.rm = T))      ## don't sum to 1...


mock1.all %>% group_by(community) %>% 
  summarize(total = sum(mock1.mean, na.rm = T))     ## sums to 1 

raw.all %>% group_by(community) %>% 
  summarize(total = sum(raw.mean, na.rm = T))      ## sums to 1 

raw.raw %>% group_by(community) %>% 
  summarize(total = sum(simple.Mean, na.rm = T))
```
