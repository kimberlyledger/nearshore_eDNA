...
mock_metadata <- read.csv("/home/kimberly.ledger/nearshore_eDNA/data/nearshore_mock_compositions_w_conc.csv") %>%
  ...
  mutate(Common_name = ifelse(Common_name == "Chum Salmon", "Chum salmon", Common_name))

## make supplementary table 
mock_table_supp <- mock_metadata %>%
  select(Community_ID, species, Common_name,Perc_Community) %>%
  mutate(Perc_Community = round(Perc_Community*100,1)) %>%
  pivot_wider(names_from = Community_ID, values_from = Perc_Community, values_fill = 0.0) %>%
  arrange(species)

write.csv(mock_table_supp, "/home/kimberly.ledger/nearshore_eDNA/outputs/TableS2.csv")

rowSums(mock_table_supp[,3:15] > 0, na.rm = TRUE)
colSums(mock_table_supp[,3:15], na.rm = TRUE)
colSums(mock_table_supp[,3:15] > 0, na.rm = TRUE)
